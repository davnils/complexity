...
D{\'{\i}}az~{\em et al.}~\cite{DST02} introduced an algorithm for computing the number of homomorphisms extending some injective homomorphism $\phi$: $hom_{\phi}(P, H)$.
...
\begin{defn}
A \emph{stingy ordering} is an ordering $v_0, v_1, ..., v_{|T|}$ of all vertices in $V(T)$ s.t.:
\begin{itemize}
\item $v_0$ is a child node and $v_{|T|}$ is the root node
...
In order to retrieve partial results for the number of injective homomorphisms $inj_{\phi}(P, H)$ it is possible to utilize apply inclusion-exclusion, as in a theorem due to Fomin \emph{et al.}~\cite{FLRRS12}:

\begin{thm}
\label{thm:fomin}
Given some $S \subseteq V(P)$ and an injective homomorphism $\phi : S \rightarrow V(H)$ then $inj_{\phi}(P, H[S' \cup \phi(S)])$ can be computed in time $O^*(|H|^{|P|})$, for all $S' \subseteq V(H) \setminus \phi(S)$ of size $|P| - |S|$.
\end{thm}
...
, i.e. $\frac{inj(P, H)}{aut(P)} = \frac{inj(P, H)}{inj(P, P)}$ \cite{FLRRS12}.
...
\begin{defn}
\label{def:delta}
$\Delta(f, g, h) = \Sigma_{\substack{A, B, C \in U\\A \cap B = A \cap C = B \cap C = \emptyset}} f(A)g(B)h(C)$
\end{defn}
...
Combined with coefficients from a \emph{Vandermonde matrix}, given by $v_{ij} = (n - 2j)^i$, Bj\"orklund \emph{et al.}~\cite{BHKK13} prove that $\Sigma_j v_{ij}x_{j} = y_i$ holds.
...
The implementation is available as a git repository \footnote{https://github.com/davnils/count-subgraphs}.

\section{General overview}
Counting of thin subgraphs has been implemented as presented in the paper \cite{BHKK13}.
Internally it generates tree decompositions using the routine \emph{create\_join\_tree} available in the library dlib 18.5 \footnote{http://www.dlib.net}
Nice tree decompositions \cite{kintali12}, counting homomorphisms \cite{DST02}, and counting injective homomorphisms \cite{FLRRS12} have been implemented as described in the corresponding papers.
The main results \cite{BHKK13} have been implemented; both basic and simple equations being solved using a linear equation solver.

In order to verify the implemented algorithm, a test suite and naive versions have been implemented, e.g. enumerating all homomorphisms.
Verification has also been performed by aggregating all function values in definition \ref{def:delta} using the naive method of enumerating all subsets.
...
\newblock {\em Electronic Colloquium on Computational Complexity (ECCC)}, (Vol. 19, p. 126), 2012.
